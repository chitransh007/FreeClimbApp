...
(function(factory) {
    ...
    "use strict";
    var pluginName = "intlTelInput", id = 1, // give each instance it's own
												// id for namespaced event
												// handling
    defaults = {
        // automatically format the number according to the selected country
        autoFormat: false,
        // if there is just a dial code in the input: remove it on blur, and
		// re-add it on focus
        ...
        onlyCountries: [],
        // the countries at the top of the list. defaults to united states and
		// united kingdom
    ...
    // keep track of if the window.load event has fired as impossible to check
	// after the fact
    ...
    Plugin.prototype = {
        init: function() {
            ...
            // backspace issues for inputs with type=text:
			// https://code.google.com/p/chromium/issues/detail?id=184812
            // and improper key codes for keyup and keydown:
			// https://code.google.com/p/chromium/issues/detail?id=118639
            if (navigator.userAgent.match(/Android/i) && navigator.userAgent.match(/Chrome/i)) {
                this.options.autoFormat = false;
            }
            // process all the data: onlyCounties, preferredCountries,
			// defaultCountry etc
            ...
            // start all of the event listeners: autoHideDialCode, input
			// keydown, selectedFlag click
            this._initListeners();
        },
        /***********************************************************************
		 * PRIVATE METHODS
		 **********************************************************************/
        // prepare all of the country data, including onlyCountries,
		// preferredCountries and
            ...
            if (this.options.onlyCountries.length) {
                ...
                for (dialCode in newCountryCodes) {
                    if (newCountryCodes[dialCode].length > 1) {
                        var sortedCountries = [];
                        // go through all of the allCountryCodes countries for
						// this dialCode and create a new (ordered) array of
						// values (if they're in the newCountryCodes array)
        ...
        // generate all of the markup for the plugin: the selected flag overlay,
		// and the dropdown
        _generateMarkup: function() {
            ...
            // country list contains: preferred countries, then divider, then
			// all countries
            this.countryList = $("<ul>", {
        ...
        _appendListItems: function(countries, className) {
            // we create so many DOM elements, I decided it was faster to build
			// a temp string
        ...
        _setInitialState: function() {
            ...
            // if the input is not pre-populated, or if it doesn't contain a
			// valid dial code, fall back to the default country
            // Note: calling setNumber will also format the number
            if (!val || !this.setNumber(val)) {
                // flag is not set, so set to the default country
                var defaultCountry;
                // check the defaultCountry option, else fall back to the first
				// in the list
        ...
        // initialise the main event listeners: input keydown, and click
		// selected flag
        _initListeners: function() {
            ...
            // hack for input nested inside label: clicking the selected-flag to
			// open the dropdown would then automatically trigger a 2nd click on
			// the input which would close it again
            var label = this.telInput.closest("label");
            if (label.length) {
                label.on("click" + this.ns, function(e) {
                    // if the dropdown is closed, then focus the input, else
					// ignore the click
            ...
            if (this.options.autoFormat) {
                ...
                // use keypress event as we want to ignore all input except for
				// a select few keys,
                // but we dont want to ignore the navigation keys like the
				// arrows etc.
                // NOTE: no point in refactoring this to only bind these
				// listeners on focus/blur because then you would need to have
				// those 2 listeners running the whole time anyway...
                this.telInput.on("keypress" + this.ns, function(e) {
                    // 32 is space, and after that it's all chars (not meta/nav
					// keys)
                    // this fix is needed for Firefox, which triggers keypress
					// event for some meta/nav keys
                    if (e.which >= keys.SPACE) {
                        ...
                        // still reformat even if not an allowed key as they
						// could by typing a formatting char, but ignore if
						// there's a selection as doesn't make sense to replace
						// selection with illegal char and then immediately
						// remove it
            ...
            // handle keyup event
            // for autoFormat: we use keyup to catch delete events after the
			// fact
            this.telInput.on("keyup" + this.ns, function(e) {
                if (that.options.autoFormat) {
                    ...
                    // if ctrl and no selection (i.e. could be paste): format
					// with suffix
                    ...
                    if (val.substr(0, 1) != "+") {
                        // newCursorPos is current pos + 1 to account for the
						// plus we are about to add
            ...
            selectedFlag.on("click" + this.ns, function(e) {
                ...
                // else let it bubble up to the top ("click-off-to-close"
				// listener)
                // we cannot just stopPropagation as it may be needed to close
				// another instance
            ...
            if (this.options.utilsScript && !$.fn[pluginName].injectedUtilsScript) {
                ...
                // if the plugin is being initialised after the window.load
				// event has already been fired
                if (windowLoaded) {
                    injectUtilsScript();
                } else {
                    // wait until the load event so we don't block any other
					// requests e.g. the flags image
                    $(window).load(injectUtilsScript);
                }
            }
        },
        // when autoFormat is enabled: handle various key events on the input:
		// the 2 main situations are 1) adding a new number character, which
		// will replace any selection, reformat, and try to preserve the cursor
		// position. and 2) reformatting on backspace, or paste event
        _handleInputKey: function(newNumericChar, addSuffix) {
            var val = this.telInput.val(), newCursor = null, cursorAtEnd = false, // raw
																					// DOM
																					// element
            ...
            if (this.isGoodBrowser) {
                ...
                // if handling a new number character: insert it in the right
				// place and calculate the new cursor position
                if (newNumericChar) {
                    // replace any selection they may have made with the new
					// char
                    val = val.substring(0, input.selectionStart) + newNumericChar + val.substring(selectionEnd, originalLen);
                    // if the cursor was not at the end then calculate it's new
					// pos
                    if (!cursorAtEnd) {
                        newCursor = selectionEnd + (val.length - originalLen);
                    }
                } else {
                    // here we're not handling a new char, we're just doing a
					// re-format, but we still need to maintain the cursor
					// position
        ...
        // on focus: if empty add dial code. on blur: if just dial code, then
		// empty it
        _initAutoHideDialCode: function() {
            var that = this;
            // mousedown decides where the cursor goes, so if we're focusing
            // we must preventDefault as we'll be inserting the dial code,
            // and we want the cursor to be at the end no matter where they
			// click
            this.telInput.on("mousedown" + this.ns, function(e) {
                if (!that.telInput.is(":focus") && !that.telInput.val()) {
                    e.preventDefault();
                    // but this also cancels the focus, so we must trigger that
					// manually
                    that.telInput.focus();
                }
            });
            // on focus: if empty, insert the dial code for the currently
			// selected flag
            this.telInput.on("focus" + this.ns, function() {
                if (!that.telInput.val()) {
                    that._updateVal("+" + that.selectedCountryData.dialCode, true);
                    // after auto-inserting a dial code, if the first key they
					// hit is '+' then assume
                    // they are entering a new number, so remove the dial code.
                    // use keypress instead of keydown because keydown gets
					// triggered for the shift key
                    // (required to hit the + key), and instead of keyup because
					// that shows the new '+'
                    ...
                    // we must use setTimeout to get outside of the focus
					// handler as it seems the
        ...
        _showDropdown: function() {
            ...
            // bind all the dropdown-related listeners: mouseover, click,
			// click-off, keydown
            this._bindDropdownListeners();
            // update the arrow
            this.selectedFlagInner.children(".arrow").addClass("up");
        },
        // decide where to position dropdown (depends on position within
		// viewport, and scroll)
        _setDropdownPosition: function() {
            var inputTop = this.telInput.offset().top, windowTop = $(window).scrollTop(), // dropdownFitsBelow
																							// =
																							// (dropdownBottom
																							// <
																							// windowBottom)
        ...
        _bindDropdownListeners: function() {
            ...
            // we add the class "highlight", so if they hit "enter" we know
			// which one to select
            ...
            // listen for up/down scrolling, enter to select, or letters to jump
			// to country name.
            // use keydown as keypress doesn't fire for non-char keys and we
			// want to catch if they
            // just hit down and hold it to scroll down (no keyup event).
            // listen on the document because that's where key events are
			// triggered if no input has focus
            ...
            $(document).on("keydown" + this.ns, function(e) {
                ...
                if (e.which == keys.UP || e.which == keys.DOWN) {
                    ...
                    // upper case letters (note: keyup/keydown only return upper
					// case letters)
        ...
        // if autoFormat=true, format it first according to the country-specific
		// formatting rules
        ...
        _updateFlag: function(number) {
            ...
            if (dialCode) {
                ...
                // countries with area codes: we must always update the flag as
				// if it's not an exact match
                // we should always default to the first country in the list.
				// This is to avoid having to
                // explicitly define every possible area code in America (there
				// are 999 possible area codes)
        ...
        // remove highlighting from other list items and highlight the given
		// item
        ...
        // the ignoreOnlyCountriesOption is only used during init() while
		// parsing the onlyCountries array
        ...
        _selectListItem: function(listItem) {
			...
			// haven't updated
            // the input val), the system as a whole has still changed - see
			// country-sync example
        ...
        // check if an element is visible within it's container, else scroll
		// until it is
        ...
        _updateDialCode: function(newDialCode) {
            ...
            if (prevDialCode.length > 1) {
                ...
                // if the previous number didn't contain a dial code, we should
				// persist it
        ...
        // try and extract a valid international dial code from a full telephone
		// number
        // Note: returns the raw string inc plus character and any
		// whitespace/dots etc
                ...
                for (var i = 0; i < inputVal.length; i++) {
                    ...
                    if ($.isNumeric(c)) {
                        ...
                        if (this.countryCodes[numericChars]) {
                            // store the actual raw string (useful for matching
							// later)
        ...
        // pass in true if you want to allow national numbers (no country dial
		// code)
        isValidNumber: function(allowNational) {
            var val = $.trim(this.telInput.val()), countryCode = allowNational ? this.selectedCountryData.iso2 : "", // libphonenumber
																														// allows
																														// alpha
																														// chars,
																														// but
        ...
        // update the selected flag, and if the input is empty: insert the new
		// dial code
        ...
        setNumber: function(number, addSuffix) {
            // we must update the flag first, which updates
			// this.selectedCountryData, which is used later for formatting the
			// number before displaying it
        ...
        utilsLoaded: function() {
            // if autoFormat is enabled and there's an initial value in the
			// input, then format it
    ...
    // Tell JSHint to ignore this warning: "character may get silently deleted
	// by one or more browsers"
    // jshint -W100
    ...
    /*
	 * var result = []; _.each(countries, function(c) { // ignore countries
	 * without a dial code if (c.callingCode[0].length) { result.push({ // var
	 * locals contains country names with localised versions in brackets n:
	 * _.findWhere(locals, { countryCode: c.cca2 }).name, i:
	 * c.cca2.toLowerCase(), d: c.callingCode[0] }); } });
    ...
    // Removed: Ã…land Islands, Christmas Island, Cocos Islands, Guernsey, Isle
	// of Man, Jersey, Kosovo, Mayotte, Pitcairn Islands, South Georgia,
	// Svalbard, Western Sahara
    // Update: converted objects to arrays to save bytes!
    // Update: added "priority" for countries with the same dialCode as others
    // Update: added array of area codes for countries with the same dialCode as
	// others
